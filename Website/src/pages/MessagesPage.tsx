...
const Avatar = ({ src, name, size = 40, online }: any) => {
  ...
  return (
    <div style={{ position: "relative", flexShrink: 0 }}>
      {safeSrc ? (
        <img src={safeSrc} alt={name} style={{ width: size, height: size, borderRadius: "50%", objectFit: "cover", border: "2px solid var(--th-accent)", opacity: 0.9 }} />
      ) : (
        <div style={{ width: size, height: size, borderRadius: "50%", background: "linear-gradient(135deg, var(--th-bg), var(--th-surface))", border: "2px solid var(--th-accent-text)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.38, fontWeight: 700, color: "var(--th-accent)", fontFamily: "'Space Grotesk', sans-serif" }}>
          {(name || "?")[0].toUpperCase()}
        </div>
      )}
      {online !== undefined && (
        <div style={{ position: "absolute", bottom: 1, right: 1, width: size * 0.27, height: size * 0.27, borderRadius: "50%", border: "2px solid var(--th-bg)", background: online ? "#4ade80" : "var(--th-muted)" }} />
      )}
    </div>
  );
};
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ width: "100%", height: 24, display: "flex", alignItems: "center", gap: 3, cursor: "pointer" }} onClick={(e) => {
          ...
          {Array.from({ length: 28 }).map((_, i) => {
            ...
            return <div key={i} style={{ flex: 1, height: h, background: isActive ? (isMine ? "#ffe792" : "#a2c2fd") : "rgba(158,172,195,0.3)", borderRadius: 2, transition: "background 0.1s" }} />;
          })}
        </div>
...
const StickerPicker = ({ onSelect, onClose }: { onSelect: (url: string) => void; onClose: () => void }) => (
  <div style={{ position: "absolute", bottom: "100%", right: 50, marginBottom: 16, zIndex: 999, background: "#031427", border: "1px solid rgba(255,231,146,0.2)", borderRadius: 16, width: 280, height: 320, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 24px 60px rgba(0,0,0,0.8)" }}>
    <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <span style={{ fontSize: 12, fontWeight: 700, color: "#ffe792" }}>STICKERS</span>
...
const EmojiPicker = ({ onSelect, onClose, direction = "up" }: { onSelect: (e: string) => void; onClose: () => void; direction?: "up" | "down" }) => {
  ...
  return (
    <div ref={containerRef} style={{ position: "absolute", left: 0, zIndex: 999, boxShadow: "0 20px 60px rgba(0,0,0,0.8)", borderRadius: 16, overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)", ...positionStyles }}>
      <ReactEmojiPicker theme={Theme.DARK} searchDisabled skinTonesDisabled width={300} height={380} onEmojiClick={(emojiData) => { onSelect(emojiData.emoji); onClose(); }} />
    </div>
  );
};
...
const NewChatModal = ({ onClose, onStartChat, currentUserId, recentChats = [] }: { onClose: () => void; onStartChat: (conv: any) => void; currentUserId: string; recentChats?: any[] }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 440, background: "var(--th-surface-low)", border: "1px solid var(--th-border)", borderRadius: 24, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 40px 100px rgba(0,0,0,0.6)", maxHeight: "85vh" }} className="glass">
        <div style={{ padding: "20px 24px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 18, fontWeight: 700, color: "var(--th-accent)", margin: 0 }}>New Transmission</h2>
        ...
        <div style={{ padding: "0 20px 14px", flexShrink: 0 }}>
          <div style={{ position: "relative" }}>
            <Icon name="search" size={18} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--th-muted)" }} />
            <input autoFocus placeholder="Search explorers..." value={query} onChange={(e) => setQuery(e.target.value)}
              style={{ width: "100%", background: "var(--th-surface-top)", border: "1px solid var(--th-border)", borderRadius: 14, padding: "11px 14px 11px 40px", fontSize: 14, color: "var(--th-text)", outline: "none", boxSizing: "border-box", fontFamily: "'Manrope', sans-serif" }} />
          </div>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "0 8px 20px" }} className="scrollbar-thin">
          {!query && recentContacts.length > 0 && (
            <div style={{ padding: "0 8px 8px" }}>
              <div style={{ fontSize: 10, color: "var(--th-muted)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8, fontWeight: 700, paddingLeft: 4 }}>Recent</div>
              {recentContacts.slice(0, 5).map((u: any) => (
                <div key={u.id || u._id} onClick={() => startChat(u)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 8px", borderRadius: 12, cursor: "pointer" }}
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, color: "var(--th-text)", fontSize: 13 }}>{u.full_name || u.username}</div>
                    <div style={{ fontSize: 11, color: "var(--th-muted)" }}>{u.uniqueTag || "@explorer"}</div>
                  </div>
                </div>
              ))}
              <div style={{ height: 1, background: "var(--th-border)", margin: "12px 4px" }} />
            </div>
          )}
          {loading && <div style={{ textAlign: "center", padding: 20, color: "var(--th-muted)", fontSize: 13 }}>Scanning...</div>}
          {!loading && query && results.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "var(--th-muted)", fontSize: 13 }}>No explorers found</div>}
          {results.map((u) => (
            <div key={u.id || u._id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 8px", borderRadius: 12, cursor: "pointer" }}
              onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(255,231,146,0.05)")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")} onClick={() => startChat(u)}>
              <Avatar src={u.avatar} name={u.full_name} size={42} online={u.isOnline} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, color: "var(--th-text)", fontSize: 14, fontFamily: "'Space Grotesk', sans-serif" }}>{u.full_name || u.username || "Unknown"}
                  {u.verified_badge && <Icon name="verified" size={14} fill style={{ color: "var(--th-accent)", marginLeft: 4, verticalAlign: "middle" }} />}
                </div>
                <div style={{ fontSize: 12, color: "var(--th-muted)", marginTop: 1 }}>{u.uniqueTag || u.email || u.username}</div>
              </div>
              {starting === (u.id || u._id) ? <div style={{ fontSize: 12, color: "var(--th-accent)" }}>Opening...</div> : <Icon name="chevron_right" size={20} style={{ color: "var(--th-muted)" }} />}
            </div>
          ))}
        </div>
      </div>
...
const NewGroupModal = ({ onClose, onStartGroup, currentUserId }: { onClose: () => void; onStartGroup: (conv: any) => void; currentUserId: string }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, background: "var(--th-surface-low)", border: "1px solid var(--th-border)", borderRadius: 24, overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: "85vh", boxShadow: "0 40px 100px rgba(0,0,0,0.6)" }} className="glass">
        <div style={{ padding: "20px 24px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 18, fontWeight: 700, color: "var(--th-accent)", margin: 0 }}>New Group</h2>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--th-muted)", padding: 4 }}><Icon name="close" size={20} /></button>
        </div>
        <div style={{ padding: "0 20px 14px", borderBottom: "1px solid var(--th-border)", flexShrink: 0 }}>
          <input placeholder="Group Name..." value={groupName} onChange={(e) => setGroupName(e.target.value)} style={{ width: "100%", background: "var(--th-surface-top)", border: "1px solid var(--th-border)", borderRadius: 12, padding: "11px 14px", fontSize: 14, color: "var(--th-text)", outline: "none", boxSizing: "border-box", marginBottom: 10 }} />
          {selectedUsers.length > 0 && <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{selectedUsers.map(u => <div key={u.id || u._id} style={{ display: "flex", alignItems: "center", gap: 4, background: "color-mix(in srgb, var(--th-accent) 15%, transparent)", color: "var(--th-accent)", padding: "3px 8px", borderRadius: 14, fontSize: 11 }}>{u.username || u.full_name?.split(' ')[0]}<Icon name="close" size={14} style={{ cursor: "pointer" }} onClick={() => toggleUser(u)} /></div>)}</div>}
        </div>
        <div style={{ padding: "12px 20px", flexShrink: 0 }}>
          <input placeholder="Search to add..." value={query} onChange={(e) => setQuery(e.target.value)} style={{ width: "100%", background: "var(--th-surface-top)", border: "1px solid var(--th-border)", borderRadius: 12, padding: "11px 14px", fontSize: 14, color: "var(--th-text)", outline: "none", boxSizing: "border-box" }} />
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "0 8px" }}>
          {results.map((u) => {
            const isSel = selectedUsers.some(x => (x.id || x._id) === (u.id || u._id)); return (
              <div key={u.id || u._id} onClick={() => toggleUser(u)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 8px", borderRadius: 12, cursor: "pointer", background: isSel ? "color-mix(in srgb, var(--th-accent) 8%, transparent)" : "transparent" }}
                onMouseEnter={(e) => !isSel && (e.currentTarget.style.background = "rgba(255,255,255,0.02)")} onMouseLeave={(e) => !isSel && (e.currentTarget.style.background = "transparent")}>
                <Avatar src={u.avatar} name={u.full_name} size={42} online={u.isOnline} />
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontWeight: 600, color: "var(--th-text)", fontSize: 14 }}>{u.full_name || u.username}</div><div style={{ fontSize: 11, color: "var(--th-muted)" }}>{u.email || u.uniqueTag}</div></div>
                {isSel ? <Icon name="check_circle" size={20} fill style={{ color: "var(--th-accent)" }} /> : <Icon name="radio_button_unchecked" size={20} style={{ color: "var(--th-muted)" }} />}
              </div>
            );
          })}
        </div>
        <div style={{ padding: 16, borderTop: "1px solid var(--th-border)", flexShrink: 0 }}>
...
const StoryModal = ({ onClose, onStoryUploaded }: { onClose: () => void; onStoryUploaded: (story: any) => void }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", backdropFilter: "blur(16px)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, background: "#031427", border: "1px solid rgba(59,73,92,0.3)", borderRadius: 24, overflow: "hidden", boxShadow: "0 40px 100px rgba(0,0,0,0.8)", display: "flex", flexDirection: "column", maxHeight: "90vh" }}>
        <div style={{ padding: "18px 22px 0", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <h2 style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 17, fontWeight: 700, color: "#ffe792", margin: 0 }}>New Signal</h2>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#9eacc3" }}><Icon name="close" size={20} /></button>
        </div>
        <div style={{ display: "flex", gap: 0, padding: "14px 22px 0", flexShrink: 0, borderBottom: "1px solid rgba(59,73,92,0.2)" }}>
          {([["media", "photo_camera", "Media"], ["audio", "mic", "Audio"], ["text", "text_fields", "Text"]] as const).map(([t, icon, label]) => (
            <button key={t} onClick={() => setTab(t as any)} style={{ flex: 1, background: "none", border: "none", borderBottom: tab === t ? "2px solid #ffe792" : "2px solid transparent", padding: "9px 0", cursor: "pointer", color: tab === t ? "#ffe792" : "#68768b", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em" }}>
        ...
        <div style={{ padding: 20, overflowY: "auto", flex: 1 }}>
          {tab === "media" && <>
            <div onClick={() => fileRef.current?.click()} style={{ border: "2px dashed rgba(255,231,146,0.2)", borderRadius: 14, minHeight: 180, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", marginBottom: 14, overflow: "hidden" }}>
              {preview ? (file?.type.startsWith("video/") ? <video src={preview} style={{ maxWidth: "100%", maxHeight: 200, borderRadius: 10 }} controls /> : <img src={preview} style={{ maxWidth: "100%", maxHeight: 200, borderRadius: 10, objectFit: "cover" }} />) : <div style={{ textAlign: "center" }}><Icon name="add_photo_alternate" size={40} style={{ color: "#ffe792", opacity: 0.5 }} /><p style={{ color: "#9eacc3", fontSize: 13, marginTop: 8 }}>Tap to select</p></div>}
            </div>
            <input ref={fileRef} type="file" hidden accept="image/*,video/*" onChange={handleFile} />
            <input placeholder="Caption (optional)" value={caption} onChange={(e) => setCaption(e.target.value)} style={{ width: "100%", background: "#071a2f", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "11px 14px", fontSize: 14, color: "#d8e6ff", outline: "none", boxSizing: "border-box" }} />
          </>}
          {tab === "text" && <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <textarea placeholder="What's on your mind?" value={textContent} onChange={(e) => setTextContent(e.target.value)} rows={3} style={{ background: "#071a2f", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "11px 14px", fontSize: 14, color: "#d8e6ff", outline: "none", resize: "none", width: "100%", boxSizing: "border-box" }} />
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{STORY_GRADIENTS.map((g) => <div key={g} onClick={() => setBgGradient(g)} style={{ width: 30, height: 30, borderRadius: 7, background: g, cursor: "pointer", border: bgGradient === g ? "2px solid #ffe792" : "2px solid transparent" }} />)}</div>
        ...
        <div style={{ padding: "0 20px 20px", flexShrink: 0 }}>
          <button onClick={submit} disabled={uploading || !canPost} style={{ width: "100%", background: uploading || !canPost ? "rgba(255,231,146,0.25)" : "linear-gradient(135deg,#ffe792,#ffc300)", color: "#655400", border: "none", borderRadius: 13, padding: "13px 0", fontWeight: 700, fontSize: 14, cursor: uploading || !canPost ? "not-allowed" : "pointer" }}>
...
export default function BubbleMessages() {
  ...
  return (
    <>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 767px) {
          .pane { position: absolute; inset: 0; width: 100%; height: 100%; transition: transform 0.32s cubic-bezier(0.4,0,0.2,1); background: var(--th-bg); }
        ...
        @media (min-width: 768px) {
          ...
          .pane-list      { width: 340px; border-right: 1px solid var(--th-border); background: var(--th-surface-low); }
          .pane-chat      { flex: 1; min-width: 0; background: var(--th-bg); }
          .pane-info      { width: 288px; border-left: 1px solid var(--th-border); background: var(--th-surface-low); overflow-y: auto; }
        }

        .msg-action-bar {
          display: flex; flex-direction: row; flex-wrap: nowrap; align-items: center; gap: 2px;
          background: rgba(3,20,39,0.98); border: 1px solid rgba(255,231,146,0.25); padding: 5px 7px;
          border-radius: 14px; box-shadow: 0 12px 40px rgba(0,0,0,0.7); backdrop-filter: blur(16px);
          z-index: 100; animation: fadeUp 0.12s ease; white-space: nowrap; min-width: max-content;
        }
        .msg-action-btn {
          display: flex; align-items: center; justify-content: center; width: 34px; height: 34px;
          border-radius: 9px; border: none; background: transparent; cursor: pointer; color: #9eacc3;
          transition: background 0.15s, color 0.15s; flex-shrink: 0;
        }
        .msg-action-btn:hover { background: rgba(255,255,255,0.07); color: #d8e6ff; }
        .msg-action-btn.danger:hover { background: rgba(239,68,68,0.12); color: #ef4444; }
        .msg-action-btn.active { color: #ffe792; }
        .msg-action-divider { width: 1px; height: 18px; background: rgba(255,255,255,0.08); margin: 0 2px; flex-shrink: 0; }
      `}</style>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 3000, background: "rgba(0,0,0,0.88)", backdropFilter: "blur(4px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ width: "100%", maxWidth: 600, padding: "16px 20px 36px", position: "relative" }}>
            <input autoFocus placeholder="Add a caption..." value={mediaCaption} onChange={(e) => setMediaCaption(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") confirmMediaUpload(); }}
              style={{ width: "100%", background: "#0b2440", border: "1px solid rgba(255,231,146,0.3)", borderRadius: 24, padding: "14px 56px 14px 20px", color: "#d8e6ff", fontSize: 15, outline: "none" }} />
            <button onClick={confirmMediaUpload} style={{ position: "absolute", right: 28, top: 24, width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,#ffe792,#ffc300)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
      ...
      {messageInfoModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }} onClick={() => setMessageInfoModal(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 380, background: "#031427", border: "1px solid rgba(59,73,92,0.3)", borderRadius: 20, overflow: "hidden", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "18px 22px 14px", borderBottom: "1px solid rgba(59,73,92,0.2)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 17, fontWeight: 700, color: "#ffe792", margin: 0 }}>Message Info</h2>
              <button onClick={() => setMessageInfoModal(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "#9eacc3" }}><Icon name="close" size={20} /></button>
            </div>
            <div style={{ padding: 22, color: "#d8e6ff", fontSize: 14, fontFamily: "'Manrope', sans-serif" }}>
              <div style={{ marginBottom: 14 }}><strong style={{ color: "rgba(255,231,146,0.8)" }}>Status: </strong><span>{messageInfoModal.readBy?.length > 0 ? "Read 👁️" : "Delivered ✓"}</span></div>
              <div style={{ marginBottom: 14 }}><strong style={{ color: "rgba(255,231,146,0.8)" }}>Time: </strong><span>{new Date(messageInfoModal.sentAt || messageInfoModal.createdAt).toLocaleString()}</span></div>
              {messageInfoModal.reactions?.length > 0 && <div><strong style={{ color: "rgba(255,231,146,0.8)" }}>Reactions:</strong><ul style={{ marginTop: 6, paddingLeft: 18, color: "#9eacc3" }}>{messageInfoModal.reactions.map((r: any, i: number) => <li key={i}>{r.emoji} by {r.user?.full_name || r.user?.username || "Unknown"}</li>)}</ul></div>}
            </div>
          </div>
        </div>
      )}
      ...
      {sessionSummary && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }} onClick={() => setSessionSummary(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 440, background: "#031427", border: "1px solid rgba(255,231,146,0.3)", borderRadius: 20, overflow: "hidden" }}>
            <div style={{ padding: "18px 22px 14px", borderBottom: "1px solid rgba(255,231,146,0.1)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Icon name="auto_awesome" style={{ color: "#ffe792" }} /><h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 17, fontWeight: 700, color: "#ffe792", margin: 0 }}>Summary</h2></div>
              <button onClick={() => setSessionSummary(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "#9eacc3" }}><Icon name="close" size={20} /></button>
            </div>
            <div style={{ padding: "24px 22px", color: "#d8e6ff", fontSize: 14, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{sessionSummary}</div>
          </div>
        </div>
      )}
      ...
      {deleteModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }}>
          <div style={{ background: "var(--th-surface-low)", border: "1px solid var(--th-border)", borderRadius: 18, padding: "24px 28px", width: "100%", maxWidth: 320, display: "flex", flexDirection: "column", gap: 14 }} className="glass">
            <h3 style={{ margin: 0, color: "var(--th-text)", fontFamily: "'Space Grotesk',sans-serif", fontSize: 17, textAlign: "center" }}>Delete Message?</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 4 }}>
              ...
              <button onClick={() => handleDeleteMessage(deleteModal.msgId, 'for_me')} style={{ background: "color-mix(in srgb, var(--th-accent) 5%, transparent)", border: "1px solid var(--th-border)", borderRadius: 10, padding: "12px", color: "var(--th-text)", fontWeight: 600, cursor: "pointer" }}>Delete for me</button>
              <button onClick={() => setDeleteModal(null)} style={{ background: "transparent", border: "none", color: "var(--th-muted)", padding: "8px", cursor: "pointer", marginTop: 4 }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {chatContextMenu && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9000 }} onClick={() => setChatContextMenu(null)} onContextMenu={(e) => { e.preventDefault(); setChatContextMenu(null); }}>
          <div onClick={(e) => e.stopPropagation()} style={{ position: "fixed", left: chatContextMenu.x, top: chatContextMenu.y, background: "#031427", border: "1px solid rgba(59,73,92,0.4)", borderRadius: 12, boxShadow: "0 20px 60px rgba(0,0,0,0.7)", overflow: "hidden", minWidth: 190, zIndex: 9001, animation: "fadeUp 0.12s ease" }}>
            {[
              ...
              <button key={item.label} onClick={item.action} style={{ width: "100%", background: "none", border: "none", padding: "11px 14px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", color: (item as any).danger ? "#ef4444" : "#d8e6ff", fontSize: 13, textAlign: "left", borderBottom: "1px solid rgba(59,73,92,0.1)" }}
                onMouseEnter={(e) => (e.currentTarget.style.background = (item as any).danger ? "rgba(239,68,68,0.08)" : "rgba(255,255,255,0.04)")} onMouseLeave={(e) => (e.currentTarget.style.background = "none")}>
                <Icon name={item.icon} size={15} style={{ color: (item as any).danger ? "#ef4444" : "#9eacc3" }} />{item.label}
              </button>
            ))}
          </div>
        </div>
      ...
      {/* ══ MAIN LAYOUT ══════════════════════════════════════════════ */}
      <div style={{ display: "flex", minHeight: "100dvh", background: "#010f20", color: "#d8e6ff", fontFamily: "'Manrope', sans-serif", overflow: "hidden" }}>
        <Sidebar />

        {/* Mobile top header — standardized with other pages */}
        <div
          className="md:hidden fixed top-0 left-0 right-0 z-30 flex items-center gap-4 px-8 h-20 border-b border-[var(--th-border)]"
          style={{ background: "color-mix(in srgb, var(--th-bg) 80%, transparent)", backdropFilter: "blur(12px)" }}
        >
          ...
          {(activePane === 'chat' || activePane === 'info' || (activePane === 'contacts' && activeChat)) ? (
            <button onClick={handleBack} style={{ background: "none", border: "none", color: "var(--th-muted)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Icon name="arrow_back" size={24} />
            </button>
          ) : (
            <button
              onClick={() => window.dispatchEvent(new CustomEvent('toggle-sidebar'))}
              style={{ background: "none", border: "none", color: "#ffe792", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
            >
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            {activePane === 'contacts' && <span style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 16, color: "var(--th-accent)", letterSpacing: "0.06em", textTransform: "uppercase" }}>Explorers</span>}
            {activePane === 'list' && <span style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 16, color: "var(--th-accent)", letterSpacing: "0.06em", textTransform: "uppercase" }}>Messages</span>}
            {activePane === 'chat' && activeChat && (
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 15, color: "var(--th-text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{getChatDisplayName(activeChat, myId)}</div>
                  <div style={{ fontSize: 10, color: "var(--th-accent)", textTransform: "uppercase", fontWeight: 600 }}>{getOtherUser(activeChat, myId)?.isOnline ? "Online" : "Offline"}</div>
                </div>
              </div>
            )}
            {activePane === 'info' && activeChat && <span style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 16, color: "var(--th-accent)", textTransform: "uppercase" }}>Details</span>}
          </div>

          {activePane === 'list' && (
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowNewChat(true)} style={{ background: "rgba(255,231,146,0.1)", border: "none", borderRadius: 10, width: 40, height: 40, cursor: "pointer", color: "#ffe792", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="chat" size={20} /></button>
              <button onClick={() => setActivePane('contacts')} style={{ background: "rgba(162,194,253,0.1)", border: "none", borderRadius: 10, width: 40, height: 40, cursor: "pointer", color: "#a2c2fd", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="contacts" size={20} /></button>
            </div>
          )}

          {activePane === 'chat' && activeChat && (
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={() => handleStartCall('voice')} style={{ width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--th-muted)" }}><Icon name="call" size={22} /></button>
              <button onClick={() => handleStartCall('video')} style={{ width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--th-muted)" }}><Icon name="videocam" size={22} /></button>
            </div>
          )}
        </div>
          ...
          <div className="msg-panes-wrapper" style={{
            ...
            <div className="pane pane-contacts" style={{ overflowY: "auto" }}>
              <div style={{ padding: "72px 20px 16px" }}>
                <h1 style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 18, fontWeight: 700, color: "var(--th-accent)", letterSpacing: "0.08em", marginBottom: 16 }}>EXPLORERS</h1>
                <div style={{ position: "relative", marginBottom: 20 }}>
                  ...
                  <input placeholder="Search identities..." onChange={async (e) => { const q = e.target.value; const res = await api.searchUsers(q); setContacts((res as any).data || res.users || []); }}
                    style={{ width: "100%", background: "var(--th-surface-top)", border: "1px solid var(--th-border)", borderRadius: 16, padding: "14px 14px 14px 44px", fontSize: 14, color: "var(--th-text)", outline: "none", boxSizing: "border-box" }} />
                </div>
              </div>
              <div style={{ padding: "0 16px 32px" }}>
                {contacts.map((u: any) => (
                  <div key={u.id || u._id} onClick={async () => { try { const res = await api.accessOrCreateChat(u.id || u._id); await handleSelectChat(res.conversation || res.data); setActivePane('chat'); } catch { toast.error("Failed"); } }}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, color: "var(--th-text)", fontSize: 15, fontFamily: "'Space Grotesk', sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.full_name || u.username}</div>
                      <p style={{ fontSize: 12, color: "var(--th-muted)", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.org_role ? `${u.org_role} @ ${u.organization || 'Explorer Hub'}` : (u.organization || 'Identity Verified')}</p>
                    </div>
                    <Icon name="chevron_right" size={20} style={{ color: "var(--th-muted)", flexShrink: 0, opacity: 0.5 }} />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pane pane-list" style={{ overflowY: "hidden", display: "flex", flexDirection: "column" }}>
              ...
              <div className="hidden md:block" style={{ padding: "24px 20px 14px", flexShrink: 0 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <h1 style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 16, fontWeight: 700, color: "var(--th-accent)", letterSpacing: "0.08em", margin: 0 }}>CHART ROOM</h1>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button title="New Chat" onClick={() => setShowNewChat(true)} style={{ background: "color-mix(in srgb, var(--th-accent) 15%, transparent)", border: "1px solid color-mix(in srgb, var(--th-accent) 20%, transparent)", borderRadius: 9, padding: 7, cursor: "pointer", color: "var(--th-accent)", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="chat" size={18} /></button>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input placeholder="Search transmissions..." value={sidebarSearch} onChange={(e) => setSidebarSearch(e.target.value)}
                    style={{ width: "100%", background: "var(--th-surface-top)", border: "1px solid var(--th-border)", borderRadius: 12, padding: "10px 14px 10px 38px", fontSize: 13, color: "var(--th-text)", outline: "none", boxSizing: "border-box" }} />
                </div>
              </div>
              ...
              <div style={{ padding: "0 16px 12px", flexShrink: 0 }}>
                <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none" }}>
                  <div onClick={() => setShowStory(true)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, flexShrink: 0, cursor: "pointer" }}>
                    <div style={{ width: 50, height: 50, borderRadius: "50%", border: "2px dashed color-mix(in srgb, var(--th-accent) 40%, transparent)", display: "flex", alignItems: "center", justifyContent: "center", background: "color-mix(in srgb, var(--th-accent) 5%, transparent)" }}>
                      <Icon name="add" size={20} style={{ color: "var(--th-accent)" }} />
                    </div>
                    <span style={{ fontSize: 9, color: "var(--th-muted)", textTransform: "uppercase" }}>Add</span>
                  </div>
                  {stories.map((s: any, i) => (
                    <div key={s._id || i} onClick={() => setViewingStory({ stories, index: i })} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, flexShrink: 0, cursor: "pointer" }}>
                      <div style={{ width: 50, height: 50, borderRadius: "50%", padding: 2, background: "linear-gradient(135deg, var(--th-accent), var(--th-secondary))", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Avatar src={s.author?.avatar} name={s.author?.full_name} size={46} />
                      </div>
                      <span style={{ fontSize: 9, color: "var(--th-muted)", maxWidth: 50, textAlign: "center", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.author?.full_name?.split(" ")[0] || "User"}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "0 8px 12px" }} className="scrollbar-thin">
                {chats.length === 0 && (
                  <div style={{ textAlign: "center", padding: "40px 20px", color: "var(--th-muted)" }}>
                  ...
                  .map((c: any) => {
                    ...
                    return (
                      <div key={c.id || c._id}
                        ...
                        style={{ display: "flex", gap: 14, alignItems: "center", padding: "16px 14px", borderRadius: 18, cursor: "pointer", marginBottom: 4, background: isActive ? "color-mix(in srgb, var(--th-accent) 7%, transparent)" : "transparent", borderLeft: isActive ? "2px solid color-mix(in srgb, var(--th-accent) 50%, transparent)" : "2px solid transparent", transition: "all 0.15s" }}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }}>
                            <span style={{ fontWeight: 700, color: isActive ? "var(--th-accent)" : "var(--th-text)", fontSize: 15, fontFamily: "'Space Grotesk', sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, paddingRight: 6 }}>{name}</span>
                            <div style={{ display: "flex", alignItems: "center", gap: 5, flexShrink: 0 }}>
                              {c.unreadCount > 0 && <span style={{ background: "var(--th-accent)", color: "var(--th-bg)", fontSize: 9, padding: "2px 6px", borderRadius: 10, fontWeight: "bold" }}>{c.unreadCount}</span>}
                              ...
                              {isPinned && !c.latestMessage?.sentAt && <Icon name="push_pin" size={11} style={{ color: "var(--th-accent)" }} />}
                            </div>
                          </div>
                          <p style={{ fontSize: 12, color: (transmitting?.typing || transmitting?.recording) ? "var(--th-accent)" : "var(--th-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", margin: 0 }}>
            ...
            {/* ══ PANE 2: Chat Thread ════════════════════════════════ */}
            <div className="pane pane-chat" style={{ display: "flex", flexDirection: "column", background: "var(--th-bg)" }}>
              {!activeChat ? (
                /* Empty state — desktop only; on mobile this pane isn't shown without a chat */
                <div className="hidden md:flex" style={{ flex: 1, flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, color: "var(--th-muted)" }}>
                  <Icon name="chat_bubble_outline" size={56} style={{ opacity: 0.12 }} />
                  <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 20, fontWeight: 600, color: "var(--th-text)", opacity: 0.35 }}>Select a transmission</h2>
                  <p style={{ fontSize: 13, opacity: 0.45 }}>Choose from the list or start a new chat</p>
                  <button onClick={() => setShowNewChat(true)} style={{ marginTop: 6, background: "var(--th-accent)", color: "var(--th-accent-text)", border: "none", borderRadius: 40, padding: "11px 22px", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>+ New Transmission</button>
                </div>
              ) : (
                <>
                  {/* Desktop Chat Header */}
                  <header className="hidden md:flex" style={{ height: 72, alignItems: "center", justifyContent: "space-between", padding: "0 24px", background: "var(--th-surface)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--th-border)", flexShrink: 0 }}>
                    {isSelectionMode || selectedMessages.size > 0 ? (
                      <>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <button onClick={() => { setSelectedMessages(new Set()); setIsSelectionMode(false); }} style={{ background: "none", border: "none", color: "var(--th-accent)", cursor: "pointer" }}><Icon name="close" size={22} /></button>
                          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--th-accent)", margin: 0 }}>{selectedMessages.size} selected</h2>
                        </div>
                        <div style={{ display: "flex", gap: 10 }}>
                          <button onClick={() => setSelectedMessages(new Set(messages.map((m: any) => m._id || m.id).filter(Boolean)))} style={{ background: "rgba(255,231,146,0.1)", border: "none", borderRadius: 9, padding: "7px 12px", cursor: "pointer", color: "var(--th-accent)", fontWeight: 600, fontSize: 12 }}>Select All</button>
                      ...
                      <>
                        <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }} onClick={() => setActivePane('info')}>
                          ...
                          <div>
                            <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 15, fontWeight: 700, color: "var(--th-text)", margin: 0 }}>{getChatDisplayName(activeChat, myId)}</h2>
                            <span style={{ fontSize: 10, color: "var(--th-accent)", textTransform: "uppercase" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <div style={{ position: "relative" }}>
                            <input placeholder="Search..." value={chatSearch} onChange={(e) => setChatSearch(e.target.value)}
                              style={{ background: "var(--th-surface-low)", border: "1px solid var(--th-border)", borderRadius: 20, padding: "7px 12px 7px 32px", fontSize: 12, color: "var(--th-text)", outline: "none", width: 130 }} />
                            <Icon name="search" size={14} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "var(--th-muted)" }} />
                          </div>
                          <button onClick={() => handleStartCall('voice')} style={{ background: "rgba(255,255,255,0.05)", border: "none", borderRadius: 10, padding: 8, cursor: "pointer", color: "var(--th-muted)", display: "flex" }}><Icon name="call" size={19} /></button>
                          <button onClick={() => handleStartCall('video')} style={{ background: "rgba(255,255,255,0.05)", border: "none", borderRadius: 10, padding: 8, cursor: "pointer", color: "var(--th-muted)", display: "flex" }}><Icon name="videocam" size={19} /></button>
                          <button onClick={loadSessionSummary} disabled={summaryLoading} style={{ background: "color-mix(in srgb, var(--th-accent) 10%, transparent)", border: "1px solid color-mix(in srgb, var(--th-accent) 20%, transparent)", borderRadius: 10, padding: 8, cursor: "pointer", color: "var(--th-accent)", display: "flex", opacity: summaryLoading ? 0.5 : 1 }}><Icon name={summaryLoading ? "hourglass_empty" : "auto_awesome"} size={19} /></button>
                        </div>
                      </>
                    )}
                  </header>
                  ...
                  <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 8px" }} className="scrollbar-thin">
                    ...
                    {messages.length === 0 && <div style={{ textAlign: "center", color: "#9eacc3", opacity: 0.5, padding: "48px 0", fontSize: 13 }}>No messages yet. Say hello! 👋</div>}
                      ...
                      return (
                        <div key={msgId || i} style={{ display: "flex", flexDirection: "column" }}>
                          <div
                            ...
                            {isPinned && <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 4, alignSelf: isMine ? "flex-end" : "flex-start" }}><Icon name="push_pin" size={10} style={{ color: "#ffe792" }} /><span style={{ fontSize: 9, color: "rgba(255,231,146,0.6)", fontWeight: 700 }}>PINNED</span></div>}
                            <div style={{ display: "flex", justifyContent: isMine ? "flex-end" : "flex-start", width: "100%", position: "relative" }}>
                              ...
                              <div style={{ maxWidth: "75%", background: isMine ? "linear-gradient(135deg, var(--th-accent), var(--th-accent))" : "var(--th-surface-low)", padding: "10px 14px", borderRadius: isMine ? "18px 18px 4px 18px" : "18px 18px 18px 4px", border: isMine ? "none" : "1px solid var(--th-border)", position: "relative", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
                                {/* 3-dots trigger */}
                                {hoveredMsg === msgId && !isSelected && actionMenuMsgId !== msgId && (
                                  <button onClick={(e) => { e.stopPropagation(); setActionMenuMsgId(msgId); }} style={{ position: "absolute", top: "50%", transform: "translateY(-50%)", ...(isMine ? { left: -34 } : { right: -34 }), background: "var(--th-surface-high)", border: "1px solid var(--th-border)", borderRadius: "50%", width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--th-accent)", cursor: "pointer", zIndex: 10 }}>
                                ...
                                {/* Reply preview */}
                                {msg.parent_message && (
                                  <div style={{ background: isMine ? "rgba(0,0,0,0.1)" : "rgba(255,255,255,0.05)", borderLeft: `3px solid ${isMine ? "rgba(0,0,0,0.3)" : "#ffe792"}`, padding: "5px 9px", borderRadius: "7px 7px 7px 2px", marginBottom: 8, fontSize: 11 }}>
                                    <span style={{ fontWeight: 800, color: isMine ? "rgba(0,0,0,0.7)" : "#ffe792", display: "block", marginBottom: 2 }}>{msg.parent_message.sender?.full_name || "Unknown"}</span>
                                    <span style={{ color: isMine ? "rgba(0,0,0,0.6)" : "#9eacc3", display: "-webkit-box", WebkitLineClamp: 1, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{msg.parent_message.content || (msg.parent_message.mediaUrl ? "📎 Attachment" : "...")}</span>
                                  </div>
                                )}
                                {/* Group sender */}
                                {activeChat.isGroupChat && !isMine && <div style={{ fontSize: 11, fontWeight: 800, color: "#ffe792", marginBottom: 3 }}>{msg.sender?.full_name || "Unknown"}</div>}
                                {/* Image */}
                                ...
                                {/* File */}
                                {isFile && (
                                  <a href={resolvedUrl!} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 11px", background: "rgba(255,255,255,0.05)", borderRadius: 10, textDecoration: "none", marginBottom: msg.content ? 6 : 0, border: "1px solid rgba(255,255,255,0.1)" }}>
                                    <div style={{ width: 34, height: 34, background: "#ffe792", borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", color: "#031427", flexShrink: 0 }}><Icon name="description" size={18} /></div>
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                      <p style={{ margin: 0, fontSize: 12, color: isMine ? "#1a0a00" : "#d8e6ff", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{msg.mediaUrl?.split('/').pop() || "File"}</p>
                                      <p style={{ margin: 0, fontSize: 10, color: isMine ? "rgba(0,0,0,0.5)" : "#68768b" }}>Download</p>
                                    </div>
                                  </a>
                                )}
                                {/* Text / Edit */}
                                {editingMsgId === msgId ? (
                                  <div style={{ display: "flex", flexDirection: "column", gap: 7, minWidth: 180 }}>
                                    <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} autoFocus style={{ width: "100%", background: "rgba(0,0,0,0.1)", border: "1px solid var(--th-border)", borderRadius: 7, padding: 7, color: isMine ? "inherit" : "var(--th-text)", fontSize: 14, outline: "none", minHeight: 55, resize: "none" }} />
                                    <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                                      <button onClick={() => setEditingMsgId(null)} style={{ background: "none", border: "none", color: "var(--th-muted)", fontSize: 10, cursor: "pointer", fontWeight: 700 }}>CANCEL</button>
                                      <button onClick={() => handleUpdateMessage(msgId, editContent, msg.sentAt || msg.createdAt)} style={{ background: isMine ? "rgba(0,0,0,0.1)" : "var(--th-accent)", border: "none", color: isMine ? "inherit" : "var(--th-accent-text)", borderRadius: 4, padding: "2px 8px", fontSize: 10, cursor: "pointer", fontWeight: 700 }}>SAVE</button>
                                    </div>
                                  </div>
                                ) : msg.content && (
                                  <div style={{ fontSize: 14, color: isMine ? "#2a1e00" : "var(--th-text)", lineHeight: 1.5, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{renderMessageText(msg.content)}</div>
                                )}
                                {/* Timestamp */}
                                <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 3, marginTop: 3, opacity: 0.75 }}>
                                  <span style={{ fontSize: 9, color: isMine ? "rgba(0,0,0,0.5)" : "#68768b", fontWeight: 600 }}>{new Date(msg.sentAt || msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                  {isMine && <Icon name={msg.readBy?.some((r: any) => (r.id || r._id || r).toString() !== myId.toString()) ? "done_all" : "check"} size={12} style={{ color: msg.readBy?.some((r: any) => (r.id || r._id || r).toString() !== myId.toString()) ? "#3b82f6" : "#9ca3af" }} />}
                                </div>
                              </div>
                            </div>
                            {/* Reactions display */}
                            {msg.reactions?.length > 0 && (
                              <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 5, alignSelf: isMine ? "flex-end" : "flex-start", marginLeft: isMine ? 0 : 38 }}>
                                {Array.from(new Set(msg.reactions.map((r: any) => r.emoji))).map((emoji: any) => {
                                  ...
                                  return (
                                    <div key={emoji} onClick={() => handleReactMessage(msgId, emoji)} style={{ background: hasReacted ? "rgba(255,231,146,0.15)" : "rgba(11,36,64,0.6)", border: `1px solid ${hasReacted ? "#ffe792" : "rgba(255,255,255,0.05)"}`, borderRadius: 9, padding: "2px 7px", display: "flex", alignItems: "center", gap: 3, cursor: "pointer" }}>
                                      <span style={{ fontSize: 12 }}>{emoji}</span>
                                      {count > 1 && <span style={{ fontSize: 9, fontWeight: 800, color: hasReacted ? "#ffe792" : "#9eacc3" }}>{count}</span>}
                                    </div>
                                  );
                                })}
                              </div>
                    ...
                    {transmittingRegistry[activeChat?.id || activeChat?._id]?.typing && (
                      <div style={{ display: "flex", justifyContent: "flex-start", marginBottom: 6, marginTop: 2, paddingLeft: 8 }}>
                        <div style={{ background: "rgba(11,36,64,0.8)", padding: "10px 14px", borderRadius: "16px 16px 16px 4px", border: "1px solid rgba(59,73,92,0.2)", display: "flex", gap: 5, alignItems: "center" }}>
                          {[0, 200, 400].map(d => <div key={d} style={{ width: 6, height: 6, borderRadius: "50%", background: "#ffe792", animation: "pulse 1s infinite", animationDelay: `${d}ms` }} />)}
                        </div>
                      </div>
                    )}
                  ...
                  {messageRequestStatus && !activeChat?.isGroupChat && (
                    <div style={{ padding: "0 16px 16px", flexShrink: 0 }}>
                      <div style={{ background: "color-mix(in srgb, var(--th-surface-low) 80%, transparent)", border: "1px solid var(--th-border)", borderRadius: 14, padding: "16px", display: "flex", flexDirection: "column", alignItems: "center", gap: 10, textAlign: "center", backdropFilter: "blur(10px)" }}>
                        <Icon name={messageRequestStatus.reason === "request_pending" && messageRequestStatus.requestDirection === "received" ? "mark_email_unread" : messageRequestStatus.reason === "request_pending" ? "hourglass_empty" : "lock"} size={28} style={{ color: messageRequestStatus.requestDirection === "received" ? "#ffe792" : "var(--th-accent)" }} />
                        <div style={{ color: "var(--th-text)", fontSize: 13, fontWeight: 600 }}>{messageRequestStatus.reason === "request_pending" && messageRequestStatus.requestDirection === "received" ? "Message Request Received" : "Message Request Required"}</div>
                        <div style={{ color: "var(--th-muted)", fontSize: 11 }}>{messageRequestStatus.reason === "request_pending" ? (messageRequestStatus.requestDirection === "sent" ? "Request sent. Waiting for acceptance." : `${getChatDisplayName(activeChat, myId)} wants to message you.`) : "Send a request to start chatting."}</div>
                        {messageRequestStatus.reason === "request_pending" && messageRequestStatus.requestDirection === "received" && (
                          <div style={{ display: "flex", gap: 10, marginTop: 2 }}>
                            <button disabled={requestSending} onClick={async () => { setRequestSending(true); try { const { respondToMessageRequest } = await import("@/api"); await respondToMessageRequest(messageRequestStatus.entityId || messageRequestStatus.requestId, "accept"); toast.success("Accepted!"); setMessageRequestStatus(null); } catch (e: any) { toast.error(e.message || "Failed"); } finally { setRequestSending(false); } }} style={{ padding: "9px 20px", background: "var(--th-accent)", color: "var(--th-accent-text)", borderRadius: 10, fontSize: 11, fontWeight: 700, cursor: "pointer", border: "none", opacity: requestSending ? 0.7 : 1 }}>{requestSending ? "..." : "✓ Accept"}</button>
                        ...
                        {messageRequestStatus.reason === "no_request" && (
                          <button disabled={requestSending} onClick={async () => { const other = getOtherUser(activeChat, myId); if (!other) return; setRequestSending(true); try { await sendRequest(other.id || other._id); toast.success("Request sent!"); setMessageRequestStatus({ reason: "request_pending", requestDirection: "sent", requestStatus: "pending" }); } catch { toast.error("Failed"); } finally { setRequestSending(false); } }} style={{ padding: "9px 20px", background: "var(--th-accent)", color: "var(--th-accent-text)", borderRadius: 10, fontSize: 11, fontWeight: 700, cursor: "pointer", border: "none", marginTop: 4, opacity: requestSending ? 0.7 : 1 }}>
                  ...
                  {!messageRequestStatus && (
                    <footer style={{ padding: "0 12px 16px", flexShrink: 0 }}>
                        ...
                        <div style={{ marginBottom: 8, padding: "0 4px" }}>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                            {aidaContext.suggestions.map((s, i) => (
                              <button key={i} onClick={() => setInputText(s)} style={{ background: "rgba(255,231,146,0.07)", border: "1px solid rgba(255,231,146,0.18)", borderRadius: 18, padding: "5px 11px", fontSize: 11, color: "#ffe792", cursor: "pointer", fontFamily: "'Manrope',sans-serif" }}
                                onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,231,146,0.14)")} onMouseLeave={e => (e.currentTarget.style.background = "rgba(255,231,146,0.07)")}>{s}</button>
                            ))}
                          </div>
                        </div>
                      )}
                      {/* Reply banner */}
                      {replyingToMsg && (
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(11,36,64,0.6)", border: "1px solid rgba(59,73,92,0.3)", borderRadius: 10, padding: "8px 12px", marginBottom: 8 }}>
                          <div style={{ overflow: "hidden" }}>
                            <span style={{ fontSize: 10, color: "#ffe792", fontWeight: 700, display: "block" }}>Replying to {replyingToMsg.sender?.full_name || "Someone"}</span>
                            <span style={{ fontSize: 12, color: "#d8e6ff", opacity: 0.7, display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 260 }}>{replyingToMsg.content || "Media"}</span>
                          </div>
                          <button onClick={() => setReplyingToMsg(null)} style={{ background: "none", border: "none", color: "#9eacc3", cursor: "pointer", padding: 4, flexShrink: 0 }}><Icon name="close" size={16} /></button>
                        </div>
                      )}
                      {/* Input row */}
                      <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(11,36,64,0.6)", border: "1px solid rgba(59,73,92,0.2)", borderRadius: 22, padding: "6px 6px 6px 14px", backdropFilter: "blur(12px)" }}>
                        {/* Emoji */}
                        <div style={{ position: "relative" }}>
                          <button onClick={() => setShowEmoji(v => !v)} style={{ background: "none", border: "none", cursor: "pointer", color: showEmoji ? "#ffe792" : "#9eacc3", padding: 5, borderRadius: 7, minWidth: 34, minHeight: 34, display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="sentiment_satisfied" size={20} /></button>
                          {showEmoji && <EmojiPicker onSelect={(emoji) => setInputText(prev => prev + emoji)} onClose={() => setShowEmoji(false)} />}
                        </div>
                        {/* Attach */}
                        <button onClick={() => fileInputRef.current?.click()} style={{ background: "none", border: "none", cursor: "pointer", color: "#9eacc3", padding: 5, borderRadius: 7, minWidth: 34, minHeight: 34, display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="attach_file" size={20} /></button>
                        ...
                        {voice.recording ? (
                          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10 }}>
                            ...
                            <span style={{ fontSize: 11, color: "#9eacc3" }}>Recording...</span>
                          </div>
                        ) : (
                          <input value={inputText} onChange={(e) => handleTyping(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                            placeholder="Type a message..." style={{ flex: 1, background: "transparent", border: "none", color: "#d8e6ff", outline: "none", fontSize: 14, fontFamily: "'Manrope', sans-serif", minHeight: 32 }} />
                        )}
                        {/* Voice btn */}
                        {!inputText.trim() && (
                          <button onMouseDown={handleVoiceStart} onClick={voice.recording ? handleVoiceSend : undefined} style={{ background: voice.recording ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.05)", border: voice.recording ? "1px solid rgba(239,68,68,0.4)" : "none", borderRadius: 11, padding: "7px 9px", cursor: "pointer", color: voice.recording ? "#ef4444" : "#9eacc3", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", minWidth: 36, minHeight: 36 }}>
                            <Icon name={voice.recording ? "stop" : "mic"} size={20} />
                          </button>
                        )}
                        {voice.recording && <button onClick={voice.cancel} style={{ background: "none", border: "none", cursor: "pointer", color: "#9eacc3", padding: 5, flexShrink: 0, display: "flex" }}><Icon name="delete" size={20} /></button>}
                        {(inputText.trim() || voice.recording) && (
                          <button onClick={voice.recording ? handleVoiceSend : handleSend} style={{ background: "linear-gradient(135deg,#ffe792,#ffc300)", border: "none", borderRadius: 14, padding: "9px 13px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 0 14px rgba(255,215,9,0.25)", minWidth: 40, minHeight: 40 }}>
            ...
            {activeChat && (
              <div className="pane pane-info" style={{ overflowY: "auto" }}>
                {(() => {
                  ...
                  const actionBtnStyle: React.CSSProperties = { display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,0.02)", border: "none", padding: "12px 13px", borderRadius: 12, color: "var(--th-muted)", fontSize: 13, fontWeight: 500, cursor: "pointer", textAlign: "left", transition: "background 0.2s", width: "100%" };
                  return (
                    <div style={{ padding: "24px 20px" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", marginBottom: 24 }}>
                        ...
                        <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 16, fontWeight: 700, color: "var(--th-text)", marginTop: 14, marginBottom: 4 }}>{getChatDisplayName(activeChat, myId)}</h2>
                        {other?.uniqueTag && <span style={{ fontSize: 11, color: "var(--th-accent)", fontFamily: "monospace" }}>{other.uniqueTag}</span>}
                        {(other?.org_role || other?.organization) && (
                          <div style={{ marginTop: 12, width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid var(--th-border)", borderRadius: 12, padding: "11px 13px", textAlign: "left" }}>
                            {other?.org_role && <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}><Icon name="badge" size={14} style={{ color: "var(--th-accent)" }} /><div><span style={{ fontSize: 9, color: "var(--th-muted)", textTransform: "uppercase", display: "block" }}>Role</span><span style={{ fontSize: 13, color: "var(--th-text)", fontWeight: 700 }}>{other.org_role}</span></div></div>}
                            {other?.organization && <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Icon name="business" size={14} style={{ color: "var(--th-muted)" }} /><div><span style={{ fontSize: 9, color: "var(--th-muted)", textTransform: "uppercase", display: "block" }}>Organization</span><span style={{ fontSize: 12, color: "var(--th-muted)", fontWeight: 500 }}>{other.organization}</span></div></div>}
                          </div>
                        )}
                        {other?.bio && <p style={{ marginTop: 12, fontSize: 12, color: "var(--th-muted)", lineHeight: 1.6, textAlign: "left", width: "100%" }}>{other.bio}</p>}
                      </div>
                      {/* Action shortcuts */}
                      <div style={{ display: "flex", justifyContent: "center", gap: 14, marginBottom: 22 }}>
                        {[{ icon: "call", label: "Call", action: () => handleStartCall('voice') }, { icon: "videocam", label: "Video", action: () => handleStartCall('video') }, { icon: "auto_awesome", label: "Summary", action: loadSessionSummary }].map(a => (
                          <div key={a.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
                            <button onClick={a.action} style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(255,255,255,0.04)", border: "1px solid var(--th-border)", cursor: "pointer", color: "var(--th-muted)", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name={a.icon} size={18} /></button>
                            <span style={{ fontSize: 9, color: "var(--th-muted)", textTransform: "uppercase" }}>{a.label}</span>
                          </div>
                        ))}
                      </div>
                      {/* Shared media */}
                      {artifacts.length > 0 && (
                        <div style={{ marginBottom: 22 }}>
                          <div style={{ fontSize: 9, color: "var(--th-muted)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10, fontWeight: 700 }}>Media ({artifacts.length})</div>
